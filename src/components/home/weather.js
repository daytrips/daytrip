        ...
        <div className="panel-body">
          ...
          <div className="col-md-12 panelBottom text-center" style={{ marginTop: '10px' }}>
            <div style={{ marginTop: '10px' }}>
              <button onClick={this.handleClick} name="prev" style={{ marginRight: '10px' }} className="btn btn-default"><a style={{ textDecoration: 'none' }} onClick={this.handleClick} name="prev" className="glyphicon glyphicon-arrow-left" /></button>
              {`${data[index].date.weekday}, ${data[index].date.pretty.slice(14)}`}
              <button onClick={this.handleClick} name="next" style={{ marginLeft: '10px' }} className="btn btn-default"><a style={{ textDecoration: 'none' }} onClick={this.handleClick} name="next" className="glyphicon glyphicon-arrow-right" /></button>
            </div>
          </div>
        </div>
      </div>
